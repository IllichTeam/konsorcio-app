...
Antes de firmar el MVP: leer el SPEC y la [revisión crítica](./ADVERSARIAL-REVIEW.md). Si todavía no se decidieron los puntos abiertos (C1, C2, C5, C6), varios tests de abajo quedan bloqueados. C3 (≥1 PDF) y C4 (`EMAIL_OVERRIDE_TO`) ya están cerrados.
...
- Elegir destinatarios, comentario, link, PDFs, contador, preview
- Que los mails lleguen con los PDFs
- Pantalla de estado del envío y “Reenviar fallidos”
- Historial del consorcio
...
5. Bucket de Storage privado listo
...
| AC-11 | Si pasás el máximo de archivos o el tamaño total acordado en el SPEC, se rechaza (cliente y servidor).                                 |
| AC-12 | **≥1 PDF obligatorio:** sin PDF no envía (CTA disabled + rechazo server).                                                              |
| AC-13 | El preview muestra el template **nuevo** (comentario, link si hay, nombres de PDFs) — no el de notificaciones.                         |
| AC-14 | El asunto del mail es el fijo del SPEC (no hay campo asunto en la UI).                                                                 |
| AC-15 | Mientras sube o envía, el botón no se puede spamear (no se crean dos envíos).                                                          |
...
| AC-22 | El saludo / identificación del destinatario usa la unidad (depto), igual que en comentario.                                                           |

### Guardado, pantalla de estado, reintentos

| ID    | ¿Qué tiene que pasar?                                                                                                 |
| ----- | --------------------------------------------------------------------------------------------------------------------- |
| AC-23 | Al enviar, se crea un registro del envío + una fila por persona; la app te lleva a la pantalla de estado.             |
| AC-24 | Esa pantalla muestra resumen (enviados / fallidos / total) y una tabla por destinatario.                              |
...
| AC-28 | En el consorcio se puede ver un historial reciente (fecha, estado, cantidades, quién envió).                          |
| AC-29 | “Reenviar fallidos” solo reintenta a los que fallaron; los que ya habían llegado **no** reciben otro mail.            |
| AC-30 | El reintento usa los mismos PDFs, comentario, link y asunto (no hace falta volver a subir archivos).                  |
| AC-31 | Si el envío se corta a mitad, la UI **no** dice “todo enviado”; se pueden continuar pendientes o reintentar fallidos. |
...
| ID    | ¿Qué tiene que pasar?                                                          |
| ----- | ------------------------------------------------------------------------------ |
| AC-38 | Textos de UI, errores y toasts en **español**; rutas en español.               |
...
3. Abrí el dialog → elegí todos → escribí comentario → adjuntá 1 o 2 PDFs chicos → mirá el preview → Enviar.
4. Deberías ir a la pantalla de estado y terminar en “enviado”.
5. En tu inbox: un mail por destinatario, con `[para: …]` en el asunto, PDFs abribles, Reply-To = email del consorcio, link en el cuerpo si lo pusiste.
...
| Demasiados PDFs o peso total de más | Rechazo claro      |
...
- Cambiá comentario / link / nombres de PDF → el preview se actualiza y se parece al mail que llega.

### Historial

- Hacé un segundo envío → la lista muestra ambos; al abrir uno volvés a la vista de estado.
...
| ~50 destinatarios                       | Termina o se puede recuperar; no deja datos incoherentes; puede tardar             |
| Corte a mitad de envío                  | No marca todo como enviado; se puede continuar/reintentar                          |
...
| Spam al botón reenviar                  | No duplica mails de más; botón deshabilitado mientras corre                        |
...
- [ ] Solo pnpm
- [ ] Código/DB en inglés; URLs en español (`/envios/`)
...
- [ ] SPEC: C1, C2, C5, C6 aplicados (C3 y C4 ya cerrados)
...
- [ ] Parcial + reintento verificado
- [ ] Reply-To verificado en al menos un mail
- [ ] Recuperación si se corta el envío (o waiver si quedó documentado otro comportamiento)
